...
@Component({
  selector: 'app-processing-bar-tlync-pay',
  templateUrl: './processing-bar-tlync-pay.page.html',
  styleUrls: ['./processing-bar-tlync-pay.page.scss'],
})
export class ProcessingBarTlyncPayPage implements OnInit {
...
counterValue: number = 0;
  ...
  ngOnInit() {
    ...
    this.startProgress();
   this.managingAppLogs("From App Step 3: Tlync Pay payment QR code generation started:",this.value.bundle.extraAmount, this.value.bundle.bundleData.name);
    this.service.confirmationLtync(this.value, this.value1).then((res: any) => {
      if (res.code == 200) {
     //   alert("Success");
        this.result = res.data[0];
        window.localStorage.setItem('L2TraveleSIM_user_result',"true");
      this.managingAppLogs("From App Step 4: Tlync Pay payment Success:",this.value.bundle.extraAmount,this.value.bundle.bundleData.name);
      } else {
       this.managingAppLogs("From App Step 4: Tlync pay payment Error:" + JSON.stringify(res),this.value.bundle.extraAmount,this.value.bundle.bundleData.name);
      }
    }).catch(err => {
 this.managingAppLogs("From App Step 4: Tlync pay payment Error from API:" + JSON.stringify(err),this.value.bundle.extraAmount,this.value.bundle.bundleData.name);   })  
  }
