...
export class ServicesService {
  ...
  */
  ...
  checkoutOrder(obj: any, access_token: any) {
    ...
    //Exchange rate API 
  exchangeRateTopupAPI(obj: any, access_token: any) {
    return new Promise((resolve, reject) => {
      const headers = new HttpHeaders()
        .set('Authorization', 'Bearer ' + access_token)
        .set('Content-Type', 'application/json; charset=utf-8')
        .set('whitelabel', this.whiteLabelId)
        .set('client-token', this.clientToken)
      return this.http.post(this.restAPI + 'exchange/currency', JSON.stringify(obj), { headers }).subscribe((res: any) => {
        resolve(res);
      }, (err) => {
        reject(err);
      });
...
debitFromAppCreditWallet(obj: any, access_token: any) {
  ...
  stripeGooglePay(obj: any, access_token: any) {
   ...
   //stripeApplePayment Apple payment
   stripeApplePayment(obj: any, access_token: any) {
    this.selectedLang = window.localStorage.getItem('L2TraveleSIM_language') == null ? 'en' : window.localStorage.getItem('L2TraveleSIM_language');
  ...
  stripeApplePayTopUpWallet(obj: any, access_token: any) {
    this.selectedLang = window.localStorage.getItem('L2TraveleSIM_language') == null ? 'en' : window.localStorage.getItem('L2TraveleSIM_language');
    ...
    return new Promise((resolve, reject) => {
      const headers = new HttpHeaders()
	   ...
	   .set('client-token', this.clientToken)
        .set('language', this.selectedLang)
      return this.http.post(this.restAPI + 'apple/pay/topup', JSON.stringify(obj), { headers }).subscribe((res: any) => {
        resolve(res);
      }, (err) => {
        reject(err);
      });
  ...
  facebookLogin(obj: any) {
    ...
    return new Promise((resolve, reject) => {
      const headers = new HttpHeaders()
        ...
        .set('language', this.selectedLang)
        .set('client-token', this.clientToken)
      return this.http.post(this.restAPI + 'facebooklogin', JSON.stringify(obj), { headers }).subscribe((res: any) => {
        resolve(res);
      }, (err) => {
        reject(err);
      });
...
validate_voucher_code(obj: any, access_token:any) {
  ...
  checkForAppUpdateIOS(obj: any) {
    return new Promise((resolve, reject) => {
      const headers = new HttpHeaders()
        .set('Content-Type', 'application/json; charset=utf-8')
        .set('whitelabel', this.whiteLabelId)
        .set('client-token', this.clientToken)
      return this.http.get(this.restAPI + 'ftelVersions/' + obj.app_platform, { headers }).subscribe((res: any) => {
        resolve(res);
      }, (err) => {
        reject(err);
      });
