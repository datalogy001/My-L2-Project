...
@Component({
  selector: 'app-processing-bar-app-creadit',
  templateUrl: './processing-bar-app-creadit.page.html',
  styleUrls: ['./processing-bar-app-creadit.page.scss'],
})
export class ProcessingBarAppCreaditPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.startProgress();
    this.managingAppLogs("From App Step 2: Wallet pay payment QR code generation started:",this.value.bundle.extraAmount, this.value.bundle.bundleData.name);
    this.service.debitFromAppCreditWallet(this.value, this.value1).then((res: any) => {
      if (res.code == 200) {
